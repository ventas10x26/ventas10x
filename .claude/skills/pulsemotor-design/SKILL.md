...
4. **Ámbar como familia de acento** (`--amber → --amber-2`, degradé tonal permitido para cifras clave y la palabra de énfasis del hero) — no mezclar otros hues como acento de marca. Verde exclusivo para estados on/off/en vivo/aprobado. El único tramo claro de la página (`.section-light`, ver "Ritmo de fondo entre secciones" en `tokens.md`) usa variantes más profundas del mismo ámbar y verde (`--amber-paper`, `--green-paper`) por contraste — sigue siendo la misma familia de acento, no un hue nuevo.
