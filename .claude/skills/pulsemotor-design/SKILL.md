...
4. **Ámbar como familia de acento** (`--amber → --amber-2`, degradé tonal permitido para cifras clave y la palabra de énfasis del hero) — no mezclar otros hues como acento de marca. Verde exclusivo para estados on/off/en vivo/aprobado — **con una excepción explícita**: la sección "Actividad en vivo" (`.section-live`, ver "Ritmo de fondo entre secciones" en `tokens.md`) eleva ese mismo verde a un degradé de fondo a escala hero (`--grad-green`), espejando lo que el ámbar ya hace en el hero. Es la misma semántica de "en vivo", no un segundo hue de marca — no repetir este tratamiento en otra sección ni usarlo como acento decorativo suelto.
