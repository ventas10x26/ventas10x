...
type VehiculoMedia = {
  linea: string; version: string; año: number; precio: number; bono: number
  fichaTecnica: string; imagenUrl: string; specs: string; combustible: string
}
...
async function leerConversacion(instanceName: string, remoteJid: string): Promise<{
  historial: MensajeHistorial[]; modeloDetectado: string | null; mediaEnviada: string[]
}> {
  try {
  ...
  } catch (e) { console.error('[webhook] leerConversacion error:', e); return { historial: [], modeloDetectado: null, mediaEnviada: [] } }
}

async function guardarConversacion(instanceName: string, remoteJid: string, historial: MensajeHistorial[], modeloDetectado: string | null, mediaEnviada: string[]) {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabaseAdmin.from('pulse_conversaciones') as any).upsert({
      instance_name: instanceName, remote_jid: remoteJid,
      historial: historial.slice(-12), modelo_detectado: modeloDetectado,
      media_enviada: mediaEnviada, updated_at: new Date().toISOString(),
    }, { onConflict: 'instance_name,remote_jid' })
  } catch (e) { console.error('[webhook] guardarConversacion error:', e) }
}

// ── CRÉDITOS ──────────────────────────────────────────────────────────────────

async function consumirCredito(instanceName: string, remoteJid: string): Promise<'ok' | 'agotado' | 'pagado' | 'error'> {
  try {
    const emailFromInst = (() => {
      const p = instanceName.split('_at_')
      return p.length === 2 ? p[0] + '@' + p[1].replace(/_/g, '.') : null
    })()
    if (!emailFromInst) return 'ok'

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: wl } = await (supabaseAdmin.from('pulse_waitlist') as any)
      .select('user_id, pagado, tipo_acceso')
      .ilike('email', emailFromInst)
      .maybeSingle()

    // Plan pagado → no consumir créditos
    if (!wl || wl.pagado || wl.tipo_acceso === 'pagado') return 'pagado'

    if (!wl.user_id) return 'ok'
    ...
    if (!creditos || creditos.saldo <= 0) {
      ...
      await (supabaseAdmin.from('pulse_waitlist') as any)
        .update({ bot_activo: false, tipo_acceso: 'suspendido' })
        .ilike('email', emailFromInst)
      return 'agotado'
    }
...
async function obtenerCatalogo(): Promise<VehiculoMedia[]> {
  ...
  try {
    ...
    catalogoCache = vehiculos; catalogoCacheTime = ahora
    return vehiculos
  } catch (e) { console.error('[webhook] catalogo error:', e); return [] }
}

function parsearCSV(csv: string): VehiculoMedia[] {
  const filas: string[][] = []
  let filaActual: string[] = [], celda = '', inQuotes = false
  for (let i = 0; i < csv.length; i++) {
    const ch = csv[i], siguiente = csv[i + 1]
    if (ch === '"') { if (inQuotes && siguiente === '"') { celda += '"'; i++ } else { inQuotes = !inQuotes } }
    else if (ch === ',' && !inQuotes) { filaActual.push(celda.trim()); celda = '' }
    else if ((ch === '\n' || ch === '\r') && !inQuotes) {
      if (ch === '\r' && siguiente === '\n') i++
  ...
  if (celda.trim() || filaActual.length > 0) { filaActual.push(celda.trim()); if (filaActual.some(c => c !== '')) filas.push(filaActual) }
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    vehiculos.push({
      linea: cells[idx.linea] || '', version: cells[idx.version] || '',
      año: parseInt(cells[idx.año] || '0'), precio: parseInt(cells[idx.precio]?.replace(/\D/g, '') || '0'),
      bono: parseInt(cells[idx.bono]?.replace(/\D/g, '') || '0'),
      fichaTecnica: cells[idx.ficha] || '', imagenUrl,
      specs: cells[idx.specs] || '', combustible: cells[idx.comb] || '',
    })
  }
  return vehiculos
}
...
const ALIASES_MODELO: Record<string, string> = {
  'picanto': 'new picanto', 'sorento': 'new sorento', 'stonic': 'new stonic',
}
...
function detectarModelo(textoCompleto: string, vehiculos: VehiculoMedia[]): VehiculoMedia | null {
  ...
  for (const [alias, real] of Object.entries(ALIASES_MODELO)) {
    if (lower.includes(alias) && !lower.includes(real)) textoExpandido = textoExpandido.replace(new RegExp(alias, 'g'), real)
  }
  for (const v of vehiculos) { if (textoExpandido.includes(v.linea.toLowerCase()) && textoExpandido.includes(v.version.toLowerCase())) return v }
  for (const v of vehiculos) { if (textoExpandido.includes(v.linea.toLowerCase())) return v }
  return null
}
...
function extraerNumeros(textoCompleto: string): { inicial: number; plazo: number } {
  ...
  const plazoMatch =
    t.match(/(\d+)\s*meses/i) || t.match(/a\s+(\d+)\s+meses/i) || t.match(/plazo\s+(?:de\s+)?(\d+)/i) ||
    t.match(/financiar\s+(?:a\s+)?(\d+)/i) || t.match(/cuotas?\s+(?:de\s+)?(\d+)\s*meses/i) ||
    (t.trim().match(/^\s*(\d+)\s*$/) && parseInt(t.trim()) >= 12 && parseInt(t.trim()) <= 120 ? t.trim().match(/^\s*(\d+)\s*$/) : null)
...
function calcularSimulacion(modelo: VehiculoMedia, inicial: number, plazo: number): string | null {
  ...
  return [
    ...
    'Inicial: ' + fmt(inicial), 'Monto a financiar: ' + fmt(monto),
    'Plazo: ' + plazo + ' meses', 'Cuota aprox: ' + fmt(cuota) + '/mes',
...
async function registrarCita(remoteJid: string, instanceName: string, diaTexto: string, horaTexto: string): Promise<void> {
  try {
    const telefono = remoteJid.replace('@s.whatsapp.net', '').replace(/^57/, '')
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: leads } = await (supabaseAdmin.from('pulse_leads') as any)
      .select('id, vendedor_id, nombre').or('telefono.ilike.%' + telefono + '%')
      .order('created_at', { ascending: false }).limit(1)
    const lead = leads?.[0]
    if (!lead) { console.log('[cita] lead no encontrado para:', telefono); return }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabaseAdmin.from('pulse_citas') as any).upsert({
      lead_id: lead.id, vendedor_id: lead.vendedor_id, remote_jid: remoteJid,
      dia_texto: diaTexto, hora_texto: horaTexto, estado: 'pendiente',
      updated_at: new Date().toISOString(),
    }, { onConflict: 'lead_id' })
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabaseAdmin.from('pulse_leads') as any)
      .update({ estado: 'test_drive', updated_at: new Date().toISOString() }).eq('id', lead.id)
    console.log('[cita] ✅ registrada — lead:', lead.nombre, '|', diaTexto, horaTexto)
  } catch (e) { console.error('[cita] error:', e) }
}
...
function extraerDiaHora(historial: { role: string; content: string }[]): { dia: string; hora: string } {
  ...
  const horaMatch = textoHistorial.match(/(\d{1,2})\s*(?:pm|am)/i) || textoHistorial.match(/(\d{1,2})\s*(?:de la tarde|de la mañana)/i)
  const hora = horaMatch ? horaMatch[0] : (textoHistorial.includes('mañana') ? 'en la mañana' : textoHistorial.includes('tarde') ? 'en la tarde' : '')
  return { dia, hora }
}
...
async function enviarBotonesTestDrive(instanceName: string, remoteJid: string, nombreModelo: string) {
  try {
    const res = await fetch(EVO_URL + '/message/sendButtons/' + instanceName, {
      method: 'POST', headers: { 'Content-Type': 'application/json', apikey: EVO_KEY },
      body: JSON.stringify({
        number: remoteJid, title: '¿Quieres conocerlo en persona?',
    ...
    if (!res.ok) {
      await enviarTexto(instanceName, remoteJid, '¿Te gustaría agendar un Test Drive del ' + nombreModelo + '?\n\nResponde:\n1️⃣ Sí, quiero agendar\n2️⃣ Aún no me decido')
    }
  } catch (e) {
...
async function enviarFicha(instanceName: string, remoteJid: string, url: string, linea: string, año: number) {
  try {
    await fetch(EVO_URL + '/message/sendMedia/' + instanceName, {
      method: 'POST', headers: { 'Content-Type': 'application/json', apikey: EVO_KEY },
      body: JSON.stringify({
        number: remoteJid, mediatype: 'document', media: url,
...
async function obtenerConfigAgente(instanceName: string): Promise<{ systemPrompt: string; nombre: string; botActivo: boolean }> {
  ...
  try {
    ...
    const { data: agente } = await (supabaseAdmin.from('pulse_agentes') as any)
      .select('bot_activo').eq('instance_name', instanceName).maybeSingle()
    ...
    let systemPrompt = '', nombre = 'el asesor'

    if (emailFromInstance) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: row } = await (supabaseAdmin.from('pulse_waitlist') as any)
        .select('email, nombre, metadata, tipo_acceso, pagado')
        .ilike('email', emailFromInstance).maybeSingle()

      if (row) {
        // Verificar si está suspendido por créditos
        if (row.tipo_acceso === 'suspendido' && !row.pagado) {
          console.log('[webhook] bot SUSPENDIDO por créditos agotados:', emailFromInstance)
          return { ...def, botActivo: false }
        }
        ...
        console.log('[webhook] config encontrada para:', emailFromInstance)
      ...
      if (res.ok) {
        ...
        if (match) {
          ...
          const { data: rows } = await (supabaseAdmin.from('pulse_waitlist') as any)
            .select('email, nombre, metadata').not('metadata', 'is', null)
          for (const row of (rows || [])) {
            ...
            if (w && w === corto) {
              ...
              return { systemPrompt: String(cfg?.system_prompt || ''), nombre: row.nombre || 'el asesor', botActivo: true }
            }
          }
        }
      }
    ...
    return def
  } catch (e) { console.error('[webhook] obtenerConfigAgente error:', e); return def }
}

// ── ANTHROPIC ─────────────────────────────────────────────────────────────────

async function generarRespuesta(texto: string, systemPrompt: string, nombre: string, historial: MensajeHistorial[], catalogoTexto: string): Promise<string | null> {
  ...
  } catch (e) { console.error('[webhook] generarRespuesta error:', e); return null }
}
  ...
  try {
    ...
    for (const msg of msgs) {
      ...
      const { data: leadCheck } = await (supabaseAdmin.from('pulse_leads') as any)
        .select('id').or('telefono.ilike.%' + telefonoRaw + '%,telefono.ilike.%57' + telefonoRaw + '%')
        .limit(1).maybeSingle()
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: convCheck } = await (supabaseAdmin.from('pulse_conversaciones') as any)
        .select('id').eq('remote_jid', remoteJid).eq('instance_name', instanceName).maybeSingle()
      ...
      const esConfirmacionTestDrive =
        btnId === 'btn_test_drive_si' || textoLower === '1' || textoLower === '1.' ||
      ...
      const esNegacionTestDrive =
        btnId === 'btn_test_drive_no' || textoLower === '2' || textoLower === '2.' ||
      ...
      if (hayOfertaTestDrive && esConfirmacionTestDrive && !yaHayDiaAcordado) {
        ...
        await enviarTexto(instanceName, remoteJid, '¡Perfecto, te espero en el concesionario! 🤝 ¿Qué día de esta semana te viene mejor para el test drive?')
        await guardarConversacion(instanceName, remoteJid, [...historial, { role: 'user', content: texto }, { role: 'assistant', content: '¡Perfecto, te espero en el concesionario! ¿Qué día te viene mejor?' }], modeloPersistido || null, mediaEnviada)
        console.log('[webhook] lead confirmó test drive')
        continue
      }
      if (hayOfertaTestDrive && esNegacionTestDrive) {
        await new Promise(r => setTimeout(r, 800))
        await enviarTexto(instanceName, remoteJid, 'Sin afán, cuando quieras. ¿Hay algo más del vehículo que quieras conocer para terminar de decidirte?')
        await guardarConversacion(instanceName, remoteJid, [...historial, { role: 'user', content: texto }, { role: 'assistant', content: 'Sin afán. ¿Hay algo más que quieras conocer?' }], modeloPersistido || null, mediaEnviada)
        continue
      }
      ...
      const modeloDetectado            = modeloDetectadoPorTexto ?? (() => {
        ...
        if (modeloPersistido.includes('|||')) {
          ...
          return vehiculos.find(v => v.linea === linea && v.version === version) ?? vehiculos.find(v => v.linea === linea) ?? null
        }
        return vehiculos.find(v => v.linea === modeloPersistido) ?? vehiculos.find(v => v.linea.includes(modeloPersistido) || modeloPersistido.includes(v.linea)) ?? null
      })()
      ...
      if (modeloDetectado && inicial > 0 && plazo > 0) simulacion = calcularSimulacion(modeloDetectado, inicial, plazo)

      const respuesta = await generarRespuesta(texto, systemPrompt, nombre, historial, catalogoTexto)

      // ── CONSUMIR CRÉDITO (solo si hay respuesta y no plan pagado) ──
      if (respuesta) {
        const estadoCredito = await consumirCredito(instanceName, remoteJid)
        if (estadoCredito === 'agotado') {
          console.log('[webhook] créditos agotados — deteniendo respuesta')
          await enviarTexto(instanceName, remoteJid,
            'En un momento mi asesor te contacta directamente. ¡Gracias por tu interés! 🙌')
          continue
        }
      }
      ...
      if (tieneDia && tieneHora) {
        ...
        const { data: citaExistente } = await (supabaseAdmin.from('pulse_citas') as any)
          .select('id').eq('remote_jid', remoteJid).maybeSingle()
        if (!citaExistente) {
          const { dia, hora } = extraerDiaHora(nuevoHistorial)
          if (dia && hora) await registrarCita(remoteJid, instanceName, dia, hora)
        }
      }
      ...
      if (simulacion) {
        ...
        const nombreModelo = modeloDetectado ? 'KIA ' + modeloDetectado.linea + ' ' + modeloDetectado.version + ' ' + modeloDetectado.año : 'el vehículo'
...
export async function GET() {
  return NextResponse.json({ ok: true, service: 'pulse-whatsapp-webhook-v27-creditos' })
}
