...
export default function NuevoBotPage() {
  ...
  const set = (key: keyof BotConfig, val: string) =>
  ...
  const saveBot = async () => {
    ...
    try {
      ...
      alert('Error al guardar el bot. Intenta de nuevo.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div style={{ minHeight: '100vh', background: '#f4f6f9', fontFamily: "var(--font-jakarta,'Plus Jakarta Sans',system-ui,sans-serif)" }}>
      ...
      <div style={{ background: '#0f1c2e', borderBottom: '1px solid rgba(255,255,255,.08)', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ color: 'rgba(255,255,255,.6)', fontSize: '14px' }}>Crear Bot IA</span>
        </div>
        <Link href="/dashboard" style={{ fontSize: '13px', color: 'rgba(255,255,255,.5)', textDecoration: 'none' }}>← Volver al dashboard</Link>
      </div>
            ...
            <div key={s} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : 'none' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                <div style={{
                  ...
                  background: i < step ? '#FF6B2B' : i === step ? '#FF6B2B' : '#e5e7eb',
        ...
        <div style={{ background: '#fff', borderRadius: '20px', border: '0.5px solid #e5e7eb', padding: '2.5rem', boxShadow: '0 2px 12px rgba(0,0,0,.06)' }}>
          ...
          {step === 0 && (
            <div>
              ...
              <p style={{ fontSize: '15px', color: '#6b7280', marginBottom: '2rem' }}>Dale un nombre y empresa a tu asistente IA.</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div>
                  ...
                  <textarea style={{ ...inputStyle, minHeight: '120px', resize: 'vertical' }}
                    placeholder="ej. Vendemos vehículos Kia nuevos: EV6 2027 desde $42,900, Sportage desde $28,500, Seltos desde $22,000. Ofrecemos financiación con Bancolombia y Davivienda, y aceptamos retomas..."
                ...
                <div>
                  ...
                  <textarea style={{ ...inputStyle, minHeight: '100px', resize: 'vertical' }}
                    placeholder="ej. ¿Hacen retomas? Sí, evaluamos tu vehículo sin costo. ¿Cuál es el tiempo de entrega? Entre 15 y 30 días hábiles..."
          ...
          {step === 4 && (
            <div>
              <h2 style={{ fontSize: '24px', fontWeight: 800, color: '#111827', marginBottom: '.5rem' }}>¡Tu bot está listo para probar!</h2>
              <p style={{ fontSize: '15px', color: '#6b7280', marginBottom: '1.5rem' }}>Chatea con <strong>{config.nombre || 'tu bot'}</strong> antes de activarlo.</p>
              ...
              {/* Resumen config */}
              <div style={{ background: '#f9fafb', border: '0.5px solid #e5e7eb', borderRadius: '14px', padding: '1.25rem', marginBottom: '1.5rem' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#374151', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '.06em' }}>Resumen de configuración</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem' }}>
                  {[
                    { label: 'Nombre', val: config.nombre },
                    { label: 'Empresa', val: config.empresa },
                    { label: 'Industria', val: config.industria },
                    { label: 'Tono', val: config.tono },
                    { label: 'WhatsApp', val: config.whatsapp || '—' },
                  ].map(r => (
                    <div key={r.label}>
                      <div style={{ fontSize: '11px', color: '#9ca3af', fontWeight: 600 }}>{r.label}</div>
                      <div style={{ fontSize: '14px', color: '#111827', fontWeight: 600, marginTop: '2px' }}>{r.val || '—'}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {saved ? (
                <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '12px', padding: '1rem 1.5rem', color: '#166534', fontWeight: 600, fontSize: '15px' }}>
                  ✅ Bot guardado exitosamente. <Link href="/dashboard" style={{ color: '#FF6B2B', textDecoration: 'none' }}>Ir al dashboard →</Link>
                </div>
              ) : (
                <button onClick={saveBot} disabled={saving} style={{ width: '100%', background: '#FF6B2B', color: '#fff', border: 'none', borderRadius: '12px', padding: '16px', fontSize: '16px', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? .7 : 1 }}>
                  {saving ? 'Guardando...' : '🚀 Activar mi Bot IA →'}
                </button>
              )}
            </div>
          )}
