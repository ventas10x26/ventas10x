...
// Una barra por etapa con su propio color (excepción acotada a esta sección, igual que los
// chips de categoría — ver "Barras del embudo" en tokens.md): a diferencia del resto del
// sistema, acá el color codifica identidad de etapa, no marca, así que el monocromático no
// aplica. Paleta tomada 1:1 de la referencia del usuario ("Funnel CRM en tiempo real").
export const EMBUDO_ETAPAS = [
  { etapa:'Oportunidades', valor:100, conv:null as string | null, grad:'linear-gradient(90deg, #4338CA, #6366F1)' },
  { etapa:'Citas',         valor:55,  conv:'55% agenda',          grad:'linear-gradient(90deg, #0E7490, #22D3EE)' },
  { etapa:'Show Up',       valor:38,  conv:'70% asiste',          grad:'linear-gradient(90deg, #7E22CE, #A855F7)' },
  { etapa:'Cotizaciones',  valor:30,  conv:'79% cotiza',          grad:'linear-gradient(90deg, #15803D, #4ADE80)' },
  { etapa:'Pedidos',       valor:12,  conv:'40% cierra',          grad:'linear-gradient(90deg, #92400E, #D97706)' },
  { etapa:'Matrículas',    valor:9,   conv:'75% matricula',       grad:'linear-gradient(90deg, #A21CAF, #E879F9)' },
]
          ...
          {EMBUDO_ETAPAS.map((e,i) => (
            <div key={e.etapa} className={`reveal${funnel.inView?' in':''}`} style={{ display:'grid', gridTemplateColumns:'130px 1fr 120px', alignItems:'center', gap:'16px', transitionDelay:`${i*90}ms` }}>
              ...
              <div style={{ position:'relative', height:'32px', background:'var(--bg-1)', border:'1px solid var(--line)', borderRadius:'6px', overflow:'hidden' }}>
                <div style={{ position:'absolute', top:0, bottom:0, left:0, right:`${funnel.inView ? 100 - (e.valor / max) * 100 : 100}%`, background:e.grad, borderRadius:'6px', display:'flex', alignItems:'center', justifyContent:'flex-end', paddingRight:'10px', transition:`right .8s var(--ease-out-expo) ${i*90}ms` }}>
