      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px 100px' }}>
        <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button type="button" onClick={() => guardar(false)} disabled={guardando || regenerando || sinAgente} style={{ padding: '10px 18px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #f97316, #ea580c)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: sinAgente ? 'not-allowed' : 'pointer', opacity: sinAgente ? 0.5 : 1, fontFamily: 'inherit' }}>
              {guardando ? 'Guardando…' : 'Guardar cambios'}
            </button>
            <button type="button" onClick={() => guardar(true)} disabled={guardando || regenerando || sinAgente} style={{ padding: '10px 18px', borderRadius: 10, border: '1px solid #f97316', background: 'rgba(249,115,22,0.08)', color: '#fdba74', fontWeight: 700, fontSize: 13, cursor: sinAgente ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
        ...
        {!sinAgente && (
          <>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20, borderBottom: '1px solid #1e293b', paddingBottom: 8 }}>
              {tabs.map((t) => (
                <button key={t.id} type="button" onClick={() => setTab(t.id)} style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: tab === t.id ? 'rgba(249,115,22,0.2)' : 'transparent', color: tab === t.id ? '#fff' : '#94a3b8', fontWeight: 700, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                  {t.label}
                </button>
              ))}
            </div>
            ...
            {tab === 'voz' && (
              <>
                <Section title="Muestra de voz del asesor" badge="ENTRENAMIENTO">
                  ...
                  <PulseVozRecorder value={form.muestra_voz} guion={guion} subiendo={subiendoVoz} onChange={(texto, dur) => patch({ muestra_voz: texto, duracion_voz_seg: dur })} onRecordingComplete={subirMuestraVoz} />
                </Section>
            ...
            {tab === 'mensajes' && (
              <>
                ...
                <Section title="Primer mensaje sugerido (WhatsApp)">
                  ...
                  <button type="button" onClick={() => { if (!form.whatsapp) { alert('Configura tu WhatsApp en la pestaña Avanzado'); return }; window.open(`https://wa.me/${form.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(form.primer_mensaje)}`, '_blank') }} style={{ marginTop: 12, padding: '10px 16px', borderRadius: 10, border: 'none', background: '#22c55e', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            {tab === 'avanzado' && (
              <>
                ...
                <Section title="System prompt interno" badge="AVANZADO">
                  <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 10px' }}>Instrucciones que Claude usa al responder leads. Edítalo solo si sabes lo que haces.</p>
                ...
                {form.agent_id && <p style={{ fontSize: 11, color: '#475569' }}>ID agente (pulse_waitlist): {form.agent_id}</p>}
              </>
            )}

            <PulseAudioLogs muestras={form.voz_historial} email={form.email} onRefresh={() => cargarAgente(form.email || undefined)} />
          </>
        )}

        <div style={{ marginTop: 24, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <button type="button" onClick={() => router.push('/pulse/dashboard')} style={{ padding: '10px 16px', borderRadius: 10, border: '1px solid #334155', background: 'transparent', color: '#94a3b8', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
            ← Dashboard de leads
          </button>
          <button type="button" onClick={() => router.push('/pulse/onboarding-demo')} style={{ padding: '10px 16px', borderRadius: 10, border: '1px solid #334155', background: 'transparent', color: '#94a3b8', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
            Repetir onboarding
          </button>
        </div>
      </div>
