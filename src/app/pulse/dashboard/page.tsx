...
//
// Mismo dashboard pero envuelto en PulseAppShell (sidebar lateral).
...
const ESTADO_LABELS: Record<string, { label: string; color: string }> = {
  ...
  interesado: { label: 'Interesado', color: '#f59e0b' },
    ...
    <PulseAppShell userName={user.nombre} userEmail={user.email}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: '#94a3b8', margin: 0 }}>
              Hola, {user.nombre.split(' ')[0]}. {leads.length === 0 ? 'Empezá creando tu primer lead.' : `Tenés ${leads.length} lead${leads.length === 1 ? '' : 's'} para gestionar.`}
            </p>
          </div>
          <button
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '24px' }}>
          <Metrica label="Leads totales" valor={leads.length} color="#fff" />
          <Metrica label="Sin contactar" valor={leads.filter(l => l.estado === 'nuevo').length} color="#fdba74" />
          <Metrica label="En seguimiento" valor={leads.filter(l => ['contactado','interesado','test_drive'].includes(l.estado)).length} color="#60a5fa" />
          <Metrica label="Cerrados" valor={leads.filter(l => l.estado === 'cerrado').length} color="#4ade80" />
        </div>

        {/* Filtros */}
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '16px' }}>
          {['todos', 'nuevo', 'contactado', 'interesado', 'test_drive', 'cerrado', 'perdido'].map((e) => (
            <button
              key={e}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {leadsFiltrados.map((lead) => {
              ...
              return (
                <div
                  key={lead.id}
                  onClick={() => { setLeadActivo(lead); setMensajeEditable(lead.mensaje_ia || ''); }}
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: '8px', cursor: 'pointer' }}
                >
        ...
        {leadActivo && (
          <div onClick={cerrarModal} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 50 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '24px', maxWidth: '640px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                <div>
                  <h3 style={{ fontSize: '20px', fontWeight: 800, margin: '0 0 4px' }}>{leadActivo.nombre || '⚠️ Lead sin clasificar'}</h3>
              ...
              {!leadActivo.nombre && (
                <button onClick={() => clasificarLead(leadActivo)} disabled={!!procesandoIA} style={btnPrimaryStyle}>
                  {procesandoIA === 'clasificar' ? '🔮 Clasificando con IA...' : '🔮 Clasificar lead con IA'}
                </button>
              )}

              {leadActivo.nombre && !leadActivo.mensaje_ia && (
                <button onClick={() => generarMensaje(leadActivo)} disabled={!!procesandoIA} style={btnPrimaryStyle}>
              ...
              {leadActivo.mensaje_ia && (
                <>
                  ...
                  <textarea value={mensajeEditable || leadActivo.mensaje_ia} onChange={(e) => setMensajeEditable(e.target.value)} rows={5} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.3)', color: '#fff', fontSize: '14px', fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box', marginBottom: '12px' }} />
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {leadActivo.telefono && (
                      <button onClick={abrirWhatsApp} style={{ ...btnPrimaryStyle, flex: 1, background: '#25D366', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginBottom: 0 }}>
                        📤 Enviar por WhatsApp
                      </button>
                    )}
                    <button onClick={() => generarMensaje(leadActivo)} disabled={!!procesandoIA} style={{ padding: '11px 16px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>
                      🔄 Regenerar
                    </button>
                  </div>
                </>
