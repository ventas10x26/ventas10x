    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root { --blue:#000; --green:#000; --grad:#000; --bg:#fff; --border:rgba(0,0,0,0.14); }
        body { background: var(--bg); }

        .pm-input { width:100%; padding:13px 16px; border-radius:10px; border:1.5px solid rgba(0,0,0,0.1); background:rgba(0,0,0,0.04); color:#000; font-size:15px; font-family:${FONT_BODY}; outline:none; transition:border-color .2s,box-shadow .2s; }
        .pm-input:focus { border-color:rgba(59,130,246,0.5); box-shadow:0 0 0 4px rgba(59,130,246,0.1); }
        .pm-input.err { border-color:#f87171; }
        .pm-input::placeholder { color:#94a3b8; }
        select option { background:#e2e8f0; }

        .pm-btn { width:100%; padding:15px; border-radius:8px; border:1px solid #000; background:#000; color:#fff; font-size:16px; font-weight:600; cursor:pointer; font-family:${FONT}; letter-spacing:-.2px; box-shadow:none; transition:all .25s cubic-bezier(.22,.68,0,1.2); }
        .pm-btn:hover:not(:disabled) { opacity:.92; transform:translateY(-2px); box-shadow:0 10px 30px rgba(59,130,246,0.22), 0 6px 16px rgba(236,72,153,0.14); }
        .pm-btn:disabled { opacity:.5; cursor:not-allowed; }

        .card { background:rgba(0,0,0,0.02); border:1px solid var(--border); border-radius:12px; transition:all .25s ease; }
        .card:hover { border-color:rgba(0,0,0,0.3); transform:translateY(-3px); box-shadow:0 16px 36px rgba(0,0,0,0.08); }

        .pm-hero-glow { position:absolute; top:0; left:0; right:0; height:560px; overflow:hidden; z-index:0; pointer-events:none; }
        .pm-hero-glow::before {
          content:''; position:absolute; left:50%; top:-380px; width:1200px; height:820px; transform:translateX(-50%);
          background:
            radial-gradient(circle at 14% 22%, rgba(59,130,246,0.26), transparent 52%),
            radial-gradient(circle at 86% 30%, rgba(236,72,153,0.22), transparent 50%),
            radial-gradient(circle at 50% 78%, rgba(168,85,247,0.16), transparent 55%);
          filter: blur(110px);
          animation: glowDrift 16s ease-in-out infinite alternate;
        }
        .pm-hero-glow::after {
          content:''; position:absolute; inset:0;
          background:linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(255,255,255,0.75) 42%, #fff 78%);
        }
        @keyframes glowDrift {
        ...
        .btn-nav-hacemos { transition: opacity .2s !important; }
        .btn-nav-hacemos:hover { opacity: .8; }
        ...
        .cube-face { position:absolute; width:260px; height:260px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; padding:24px; font-size:13px; font-weight:600; font-family:${FONT_BODY}; text-align:center; line-height:1.35; }
        .cube-section-bg { position:relative; overflow:hidden; }

        .btn-hacemos { transition:opacity .2s !important; }
        .btn-hacemos:hover { opacity:.85; }
      ...
      <div style={{ minHeight:'100vh', background:'var(--bg)', color:'#000', fontFamily:FONT_BODY, position:'relative' }}>

        <div className="pm-hero-glow" />

        {/* HEADER */}
        <header style={{ position:'sticky', top:0, zIndex:100, padding:'16px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', maxWidth:'1100px', margin:'0 auto', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(0,0,0,0.05)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            <div style={{ width:'32px', height:'32px', borderRadius:'8px', background:'#000', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px' }}>⚡</div>
            <span style={{ fontSize:'18px', fontWeight:700, fontFamily:FONT, letterSpacing:'-.3px' }}>Pulse Motor</span>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            {usuarioLogueado ? (
              <a href="/pulse/agente" style={{ fontSize:'14px', fontWeight:600, color:'#fff', padding:'8px 18px', borderRadius:'8px', background:'#000', textDecoration:'none' }}>Mi agente →</a>
            ) : (
              <>
                <button onClick={() => setModalContacto(true)} className="btn-nav-hacemos" style={{ fontSize:'13px', fontWeight:600, color:'#000', padding:'8px 16px', borderRadius:'8px', background:'transparent', border:'1px solid rgba(0,0,0,0.2)', cursor:'pointer', fontFamily:FONT_BODY, letterSpacing:'-.1px' }}>
                  Lo hacemos por vos
                </button>
                <a href="#precios" style={{ fontSize:'14px', fontWeight:500, color:'#475569', padding:'8px 16px', borderRadius:'8px', textDecoration:'none', transition:'color .15s' }} onMouseEnter={e=>(e.currentTarget.style.color='#1e293b')} onMouseLeave={e=>(e.currentTarget.style.color='#475569')}>Precios</a>
                <a href="/pulse/login"  style={{ fontSize:'14px', fontWeight:500, color:'#475569', padding:'8px 16px', borderRadius:'8px', border:'1px solid rgba(0,0,0,0.1)', textDecoration:'none' }}>Ingresar</a>
                <a href="/pulse/signup" style={{ fontSize:'14px', fontWeight:600, color:'#fff', padding:'8px 18px', borderRadius:'8px', background:'#000', textDecoration:'none' }}>Registrarse →</a>
              </>
            )}
          </div>
        </header>
        ...
        <section style={{ position:'relative', zIndex:1, maxWidth:'1100px', margin:'0 auto', padding:'72px 24px 80px' }}>
          <div style={{ textAlign:'center', marginBottom:'56px' }}>
            <div style={{ ...v(100), display:'inline-flex', alignItems:'center', gap:'8px', background:'transparent', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'999px', padding:'5px 14px', fontSize:'12px', fontWeight:500, color:'#888', marginBottom:'28px' }}>
            ...
            <div style={{ ...v(1100), display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', flexWrap:'wrap' }}>
              {[
                ...
                <div key={p.text} dangerouslySetInnerHTML={{ __html:p.text }} style={{ display:'inline-flex', alignItems:'center', gap:'6px', padding:'5px 12px', borderRadius:'999px', fontSize:'12px', fontWeight:500, background:'transparent', border:'1px solid rgba(0,0,0,0.1)', color:'#666' }} />
              ))}
            </div>
          </div>

          {/* Video Synthesia */}
          <div style={{ display:'flex', alignItems:'center', gap:'64px', justifyContent:'center', flexWrap:'wrap', marginTop:'80px', paddingTop:'80px', borderTop:'1px solid rgba(0,0,0,0.05)' }}>
            <div style={{ flex:'1', minWidth:'280px', maxWidth:'420px' }}>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'7px', background:'transparent', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'999px', padding:'5px 14px', fontSize:'12px', fontWeight:500, color:'#888', marginBottom:'24px' }}>Mirá cómo funciona</div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'16px', marginBottom:'36px' }}>
                {[
                  { icon:'⚡', bg:'rgba(0,0,0,0.06)', border:'rgba(0,0,0,0.14)', text:'Responde al lead en 30 segundos desde tu número' },
                  { icon:'🔁', bg:'rgba(0,0,0,0.06)', border:'rgba(0,0,0,0.14)', text:'Follow-up automático día 1, 3 y 7 sin que hagas nada' },
                  { icon:'🎯', bg:'rgba(0,0,0,0.06)', border:'rgba(0,0,0,0.14)', text:'Entrena al agente con tu propio estilo de venta' },
                ].map(item => (
                  <div key={item.icon} style={{ display:'flex', gap:'14px', alignItems:'center' }}>
                    ...
                    <span style={{ fontSize:'14px', color:'#475569', lineHeight:'1.5' }}>{item.text}</span>
                  </div>
                ))}
              </div>
              <a href="/pulse/signup" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'13px 26px', borderRadius:'8px', background:'#000', color:'#fff', fontSize:'15px', fontWeight:600, textDecoration:'none', fontFamily:FONT, letterSpacing:'-.2px' }}>Crear mi agente gratis →</a>
              <p style={{ fontSize:'12px', color:'#cbd5e1', marginTop:'10px' }}>14 días gratis · Sin tarjeta · Tu WhatsApp actual</p>
            </div>
            <div style={{ flex:'1.2', minWidth:'300px', maxWidth:'600px' }}>
              <div style={{ position:'relative', overflow:'hidden', aspectRatio:'1920/1080', borderRadius:'20px', boxShadow:'0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(0,0,0,0.07)', border:'1px solid rgba(0,0,0,0.08)' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'10px', marginTop:'18px', flexWrap:'wrap' }}>
                {[
                  { icon:'⭐', text:'4.9/5 asesores',       color:'#666', bg:'transparent',  border:'rgba(0,0,0,0.14)' },
                  { icon:'🚗', text:'+500 leads atendidos', color:'#666', bg:'transparent', border:'rgba(0,0,0,0.14)' },
                  { icon:'⚡', text:'30 seg respuesta',     color:'#666', bg:'transparent', border:'rgba(0,0,0,0.14)' },
                ].map(b => (
          ...
          {/* Hero grid */}
          <div className="hero-grid" style={{ display:'flex', alignItems:'center', gap:'48px', justifyContent:'center', marginTop:'80px', paddingTop:'80px', borderTop:'1px solid rgba(0,0,0,0.05)' }}>
            <form onSubmit={submit} style={{ ...v(1200), flex:'1', minWidth:'280px', maxWidth:'380px', background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'20px', padding:'28px', backdropFilter:'blur(12px)' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'12px', marginBottom:'16px' }}>
                ...
                <select value={marca} onChange={e=>setMarca(e.target.value)} disabled={estado==='enviando'} className="pm-input" style={{ cursor:'pointer', color:marca?'#000':'#94a3b8' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
                {['Conecta con tu WhatsApp actual vía QR','Sin número nuevo ni SIM extra','14 días gratis sin tarjeta'].map(t=>(
                  <div key={t} style={{ fontSize:'13px', color:'#cbd5e1', display:'flex', gap:'6px', alignItems:'flex-start' }}>
                    <span style={{ color:'#000', flexShrink:0 }}>✓</span><span>{t}</span>
                  </div>
                ))}
              </div>
              <p style={{ fontSize:'13px', color:'#cbd5e1', margin:'16px 0 0', textAlign:'center' }}>
                ¿Ya tenés cuenta? <a href="/pulse/login" style={{ color:'#000', textDecoration:'underline', fontWeight:500 }}>Ingresar →</a>
              </p>
            </form>

            <div style={{ ...v(1400), flex:'1', minWidth:'280px', maxWidth:'340px', display:'flex', flexDirection:'column', alignItems:'center', gap:'16px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'7px', background:'transparent', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'999px', padding:'5px 14px', fontSize:'12px', fontWeight:500, color:'#888' }}>
              ...
              <div style={{ width:'270px', background:'#1a1a2e', borderRadius:'40px', padding:'12px', border:'2px solid rgba(255,255,255,0.12)', boxShadow:'0 24px 60px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ fontSize:'12px', color:'#cbd5e1', textAlign:'center' }}>Responde solo mientras vos cerrás otras ventas</div>
            </div>
          </div>
        </section>
        ...
        <section className="hacemos-section" style={{ position:'relative', zIndex:1, padding:'80px 24px 90px' }}>
          ...
          <div style={{ maxWidth:'1100px', margin:'0 auto', position:'relative', zIndex:1 }}>
            <div style={{ textAlign:'center', marginBottom:'56px' }}>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'transparent', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'999px', padding:'6px 18px', marginBottom:'16px' }}>
                <span style={{ fontSize:'12px', fontWeight:600, color:'#888', letterSpacing:'1px', textTransform:'uppercase' }}>Lo hacemos juntos</span>
              </div>
              <h2 style={{ fontSize:'clamp(28px,4vw,48px)', fontWeight:700, letterSpacing:'-.5px', fontFamily:FONT, marginBottom:'14px', lineHeight:'1.1' }}>
                ...
                <span style={{ color:'#888' }}>Nosotros hacemos el resto.</span>
              </h2>
              ...
              <button onClick={() => setModalContacto(true)} className="btn-hacemos" style={{ display:'inline-flex', alignItems:'center', gap:'10px', background:'#000', border:'none', borderRadius:'8px', padding:'14px 28px', fontFamily:FONT, fontSize:'16px', fontWeight:600, color:'#fff', cursor:'pointer', letterSpacing:'-.2px' }}>
                Quiero que lo hagan por mí →
              </button>
              <p style={{ fontSize:'12px', color:'#cbd5e1', marginTop:'10px' }}>Te contactamos en menos de 24 hs · Sin compromiso · Sin tarjeta</p>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:'0' }}>
              ...
              <div style={{ display:'flex', gap:'40px', alignItems:'stretch', flexWrap:'wrap' }}>
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', width:'56px', flexShrink:0 }}>
                  <div style={{ width:'56px', height:'56px', borderRadius:'50%', background:'rgba(0,0,0,0.08)', border:'1px solid rgba(0,0,0,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', fontWeight:800, color:'#000', fontFamily:FONT, flexShrink:0 }}>1</div>
                  <div style={{ width:'1px', flex:1, background:'rgba(0,0,0,0.14)', marginTop:'8px', minHeight:'60px' }} />
                </div>
                <div style={{ flex:1, minWidth:'260px', paddingBottom:'48px' }}>
                  <div style={{ display:'flex', alignItems:'flex-start', gap:'24px', flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:'220px' }}>
                      <p style={{ fontSize:'11px', fontWeight:700, color:'#888', letterSpacing:'2px', textTransform:'uppercase', margin:'0 0 8px', paddingTop:'14px' }}>Vos hacés</p>
                      ...
                      <button onClick={() => setModalContacto(true)} className="btn-hacemos" style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'#000', border:'none', borderRadius:'8px', padding:'12px 22px', fontFamily:FONT, fontSize:'14px', fontWeight:600, color:'#fff', cursor:'pointer', letterSpacing:'-.1px' }}>
                        Empezamos — escribinos ahora
                      </button>
                      <p style={{ fontSize:'12px', color:'#cbd5e1', marginTop:'8px' }}>Te contactamos en menos de 24 hs · Sin compromiso</p>
                    </div>
                    <div style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'16px', padding:'20px', minWidth:'220px', maxWidth:'280px', flexShrink:0 }}>
                      <div style={{ fontSize:'11px', color:'#666', fontWeight:600, marginBottom:'14px', letterSpacing:'1px', textTransform:'uppercase' }}>Pulse Motor te pregunta</div>
                      {[{q:'¿Cómo hablás normalmente con un cliente interesado?',tag:'Tu estilo'},{q:'¿Cuál es tu mayor obstáculo para cerrar una venta?',tag:'Tu reto'}].map((item,i) => (
                        <div key={i} style={{ background:'rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.06)', borderRadius:'10px', padding:'12px', marginBottom:'8px' }}>
                          <div style={{ display:'inline-block', fontSize:'10px', fontWeight:700, color:'#666', background:'rgba(0,0,0,0.08)', padding:'2px 8px', borderRadius:'999px', marginBottom:'6px' }}>{item.tag}</div>
                          <p style={{ fontSize:'12px', color:'#475569', lineHeight:'1.5', margin:0 }}>{item.q}</p>
                        </div>
                      ))}
                      <div style={{ marginTop:'12px', padding:'10px 12px', background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.14)', borderRadius:'8px', fontSize:'12px', color:'#444', fontWeight:600 }}>✓ Con eso creamos tu agente personalizado</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display:'flex', gap:'40px', alignItems:'stretch', flexWrap:'wrap' }}>
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', width:'56px', flexShrink:0 }}>
                  <div style={{ width:'56px', height:'56px', borderRadius:'50%', background:'rgba(0,0,0,0.08)', border:'1px solid rgba(0,0,0,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', fontWeight:800, color:'#000', fontFamily:FONT, flexShrink:0 }}>2</div>
                  <div style={{ width:'1px', flex:1, background:'rgba(0,0,0,0.14)', marginTop:'8px', minHeight:'60px' }} />
                </div>
                <div style={{ flex:1, minWidth:'260px', paddingBottom:'48px' }}>
                  <div style={{ display:'flex', alignItems:'flex-start', gap:'24px', flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:'220px' }}>
                      <p style={{ fontSize:'11px', fontWeight:700, color:'#888', letterSpacing:'2px', textTransform:'uppercase', margin:'0 0 8px', paddingTop:'14px' }}>Solo abrís la cámara</p>
                      ...
                      <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                        {['Tu número de siempre','Sin SIM nueva'].map(t => (
                          <div key={t} style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.14)', borderRadius:'10px', padding:'8px 14px', fontSize:'13px', color:'#444', fontWeight:600 }}>{t}</div>
                        ))}
                      </div>
                    </div>
                    <div style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'16px', padding:'20px', minWidth:'220px', maxWidth:'280px', flexShrink:0 }}>
                      <div style={{ fontSize:'11px', color:'#666', fontWeight:600, marginBottom:'14px', letterSpacing:'1px', textTransform:'uppercase' }}>Así de simple</div>
                      {[{icon:'📱',paso:'Abrís WhatsApp en tu celular'},{icon:'⋮',paso:'Tocás los 3 puntos → Dispositivos vinculados'},{icon:'📷',paso:'Apuntás al QR en la pantalla'},{icon:'✅',paso:'¡Listo! Tu agente está activo',ok:true}].map((s,i) => (
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 0', borderBottom:i<3?'1px solid rgba(0,0,0,0.04)':'none' }}>
                          <div style={{ width:'28px', height:'28px', borderRadius:'8px', background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.14)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'13px', flexShrink:0 }}>{s.icon}</div>
                          <span style={{ fontSize:'12px', color:s.ok?'#000':'#475569', fontWeight:s.ok?600:400 }}>{s.paso}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div style={{ display:'flex', gap:'40px', alignItems:'stretch', flexWrap:'wrap' }}>
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', width:'56px', flexShrink:0 }}>
                  <div style={{ width:'56px', height:'56px', borderRadius:'50%', background:'rgba(0,0,0,0.08)', border:'1px solid rgba(0,0,0,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', fontWeight:800, color:'#000', fontFamily:FONT, flexShrink:0 }}>3</div>
                </div>
                <div style={{ flex:1, minWidth:'260px' }}>
                  <div style={{ display:'flex', alignItems:'flex-start', gap:'24px', flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:'220px' }}>
                      <p style={{ fontSize:'11px', fontWeight:700, color:'#888', letterSpacing:'2px', textTransform:'uppercase', margin:'0 0 8px', paddingTop:'14px' }}>De acá en adelante</p>
                      ...
                      <a href="/pulse/signup" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'13px 24px', borderRadius:'8px', background:'#000', color:'#fff', fontSize:'15px', fontWeight:600, textDecoration:'none', fontFamily:FONT, letterSpacing:'-.2px' }}>Empezar ahora — es gratis →</a>
                      <p style={{ fontSize:'12px', color:'#cbd5e1', marginTop:'8px' }}>14 días gratis · Sin tarjeta · Tu WhatsApp actual</p>
                    </div>
                    <div style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'16px', padding:'16px', minWidth:'220px', maxWidth:'280px', flexShrink:0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'14px', paddingBottom:'10px', borderBottom:'1px solid rgba(0,0,0,0.05)' }}>
                        ...
                        <div><div style={{ fontSize:'12px', fontWeight:700, color:'#000' }}>Tu agente IA</div><div style={{ fontSize:'10px', color:'#888' }}>● En línea · responde solo</div></div>
                      </div>
                      {[{out:false,text:'Hola, me interesa el Sportage',sub:'Lead nuevo · 3:42pm'},{out:true,text:'¡Hola! Desde $127M. ¿Te agendo test drive? 🚗',sub:'Agente · 3:42pm ✓✓'},{out:false,text:'Sí, este sábado',sub:'Lead · 3:43pm'},{out:true,text:'✅ Listo, sábado 10am agendado',sub:'Agente · 3:43pm ✓✓'}].map((m,i) => (
                        <div key={i} style={{ display:'flex', justifyContent:m.out?'flex-end':'flex-start', marginBottom:'8px' }}>
                          <div style={{ maxWidth:'80%' }}>
                            <div style={{ padding:'7px 10px', borderRadius:m.out?'12px 12px 2px 12px':'12px 12px 12px 2px', background:m.out?'#005c4b':'rgba(0,0,0,0.06)', fontSize:'11px', color:m.out?'#fff':'#1e293b', lineHeight:'1.4' }}>{m.text}</div>
                            <div style={{ fontSize:'9px', color:'#cbd5e1', marginTop:'2px', textAlign:m.out?'right':'left' }}>{m.sub}</div>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <section style={{ position:'relative', zIndex:1, maxWidth:'1100px', margin:'0 auto', padding:'80px 24px' }}>
          <div className="skills-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'64px', alignItems:'center' }}>
            <div>
              <p style={{ fontSize:'11px', fontWeight:700, letterSpacing:'2px', color:'#888', textTransform:'uppercase', marginBottom:'16px' }}>Tu agente IA</p>
              <h2 style={{ fontSize:'clamp(26px,3vw,40px)', fontWeight:700, fontFamily:FONT, letterSpacing:'-.4px', lineHeight:'1.1', marginBottom:'40px' }}>
                Un agente que{' '}<span style={{ color:'#888' }}>vende como vos</span>
              </h2>
              <div style={{ display:'flex', flexDirection:'column' }}>
                {[
                  ...
                  <div key={h.label} style={{ display:'flex', gap:'16px', alignItems:'flex-start', padding:'16px 0', borderBottom:i<5?'1px solid rgba(0,0,0,0.05)':'none' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'10px', flexShrink:0, background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.14)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'18px' }}>{h.icon}</div>
                    <div><p style={{ fontSize:'14px', fontWeight:700, color:'#1e293b', margin:'0 0 3px' }}>{h.label}</p><p style={{ fontSize:'13px', color:'#94a3b8', margin:0, lineHeight:'1.55' }}>{h.desc}</p></div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ width:'100%', background:'rgba(0,0,0,0.025)', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', overflow:'hidden' }}>
              ...
              <div style={{ background:'rgba(0,0,0,0.02)', borderBottom:'1px solid rgba(0,0,0,0.05)', padding:'14px 18px', display:'grid', gridTemplateColumns:'repeat(3,1fr)' }}>
                {[{val:'47',label:'Leads atendidos'},{val:'89%',label:'Tasa respuesta'},{val:'8',label:'Citas agendadas'}].map((s,i) => (
                  <div key={s.label} style={{ textAlign:'center', padding:'0 12px', borderRight:i<2?'1px solid rgba(0,0,0,0.05)':'none' }}>
                    <div style={{ fontSize:'22px', fontWeight:700, fontFamily:FONT, color:'#000' }}>{s.val}</div>
                    <div style={{ fontSize:'10px', color:'#94a3b8', marginTop:'2px' }}>{s.label}</div>
                  </div>
                ))}
              </div>
              <div style={{ padding:'14px 18px', display:'flex', flexDirection:'column', gap:'10px' }}>
                <div style={{ fontSize:'11px', fontWeight:700, letterSpacing:'1.5px', color:'#94a3b8', textTransform:'uppercase', marginBottom:'4px' }}>Últimas conversaciones</div>
                {[{name:'María C.',msg:'¡Perfecto! Mañana a las 10am ✅',time:'hace 2 min',status:'cita'},{name:'Carlos R.',msg:'Desde $127M neto. ¿Te muestro opciones?',time:'hace 18 min',status:'activo'},{name:'Ana G.',msg:'Seguimiento D3: ¿pudiste ver el catálogo?',time:'hace 3h',status:'seguimiento'},{name:'Luis M.',msg:'Tu cita es mañana sábado 11am 📅',time:'ayer',status:'cita'}].map(conv => (
                  <div key={conv.name} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'10px', borderRadius:'10px', background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.04)' }}>
                    <div style={{ width:'38px', height:'38px', borderRadius:'50%', background:'rgba(0,0,0,0.08)', border:'1px solid rgba(0,0,0,0.14)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'14px', fontWeight:700, color:'#444', flexShrink:0 }}>{conv.name[0]}</div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'2px' }}><span style={{ fontSize:'13px', fontWeight:600, color:'#1e293b' }}>{conv.name}</span><span style={{ fontSize:'10px', color:'#cbd5e1' }}>{conv.time}</span></div>
                      <div style={{ fontSize:'11px', color:'#94a3b8', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{conv.msg}</div>
                    </div>
                    <div style={{ flexShrink:0, fontSize:'9px', fontWeight:700, padding:'3px 7px', borderRadius:'6px', background:'rgba(0,0,0,0.05)', color:'#999', border:'1px solid rgba(0,0,0,0.1)', textTransform:'uppercase', letterSpacing:'0.5px' }}>{conv.status}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="cube-section-bg" style={{ position:'relative', zIndex:1, padding:'100px 24px 110px' }}>
          <div style={{ maxWidth:'1100px', margin:'0 auto' }}>
            <div style={{ textAlign:'center', marginBottom:'70px' }}>
              <p style={{ fontSize:'11px', fontWeight:700, letterSpacing:'2px', color:'#888', textTransform:'uppercase', marginBottom:'14px' }}>Todo en un solo agente</p>
              <h2 style={{ fontSize:'clamp(28px,4vw,48px)', fontWeight:700, fontFamily:FONT, letterSpacing:'-.5px', lineHeight:'1.08', marginBottom:'16px' }}>6 superpoderes.{' '}<span style={gradText}>Un solo agente.</span></h2>
              <p style={{ fontSize:'16px', color:'#94a3b8', maxWidth:'460px', margin:'0 auto', lineHeight:'1.6' }}>Cada cara del cubo es una capacidad que trabaja sola, en paralelo, 24/7. Pasá el mouse para explorarlo.</p>
            </div>
            <div className="cube-layout" style={{ display:'flex', alignItems:'center', gap:'80px', justifyContent:'center' }}>
              <div style={{ display:'flex', flexDirection:'column', gap:'14px', flex:'1', maxWidth:'210px', flexShrink:0 }}>
                {CUBE_FACES.slice(0,3).map(f => (
                  <div key={f.face} style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'14px', padding:'14px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'10px', background:'rgba(0,0,0,0.06)', border:'1px solid rgba(0,0,0,0.14)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'18px' }}>{f.icon==='WA'?WA_SVG_SM:f.icon}</div>
                    <div><div style={{ fontSize:'12px', fontWeight:700, color:'#1e293b', lineHeight:1.3 }}>{f.label}</div><div style={{ fontSize:'11px', color:'#94a3b8', marginTop:'2px' }}>{f.sub}</div></div>
                  </div>
                ))}
              </div>
              <div style={{ flexShrink:0, display:'flex', flexDirection:'column', alignItems:'center', gap:'20px' }}>
                <div style={{ position:'relative', width:'520px', height:'520px', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  ...
                  <div id="pm-cube-scene" style={{ width:'260px', height:'260px', perspective:'900px', cursor:'crosshair', position:'relative', zIndex:1 }}>
                    <div id="pm-cube" style={{ width:'260px', height:'260px', position:'relative', transformStyle:'preserve-3d' as const }}>
                      {CUBE_FACES.map(f => (
                        <div key={f.face} className="cube-face" style={{ transform:f.tx, background:'rgba(250,250,250,0.97)', border:'1px solid rgba(0,0,0,0.14)', color:'#000', boxShadow:'0 4px 20px rgba(0,0,0,0.08)' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'8px', fontSize:'12px', color:'#cbd5e1' }}><span style={{ fontSize:'16px' }}>↔</span> Pasá el mouse para girarlo</div>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:'14px', flex:'1', maxWidth:'210px', flexShrink:0 }}>
                {CUBE_FACES.slice(3,6).map(f => (
                  <div key={f.face} style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'14px', padding:'14px 16px', display:'flex', alignItems:'center', gap:'12px' }}>
                    <div style={{ width:'36px', height:'36px', borderRadius:'10px', background:'rgba(0,0,0,0.06)', border:'1px solid rgba(0,0,0,0.14)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'18px' }}>{f.icon}</div>
                    <div><div style={{ fontSize:'12px', fontWeight:700, color:'#1e293b', lineHeight:1.3 }}>{f.label}</div><div style={{ fontSize:'11px', color:'#94a3b8', marginTop:'2px' }}>{f.sub}</div></div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ textAlign:'center', marginTop:'56px' }}>
              <a href="/pulse/signup" style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'14px 28px', borderRadius:'8px', background:'#000', color:'#fff', fontSize:'15px', fontWeight:600, textDecoration:'none', fontFamily:FONT, letterSpacing:'-.2px' }}>Activar los 6 superpoderes →</a>
              <p style={{ fontSize:'13px', color:'#cbd5e1', marginTop:'12px' }}>14 días gratis · Sin tarjeta</p>
            </div>
          </div>
        </section>
        ...
        <section style={{ position:'relative', zIndex:1, maxWidth:'1100px', margin:'0 auto', padding:'80px 24px' }}>
          <div style={{ textAlign:'center', marginBottom:'52px' }}>
            <p style={{ fontSize:'11px', fontWeight:700, letterSpacing:'2px', color:'#888', textTransform:'uppercase', marginBottom:'10px' }}>Cómo funciona</p>
          ...
          <div className="grid3" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px' }}>
            {PASOS.map((p,i) => (
              <div key={p.num} className="card" style={{ padding:'26px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ fontSize:'11px', fontWeight:700, letterSpacing:'2px', color:'#888', textTransform:'uppercase', marginBottom:'10px' }}>{p.num}</div>
                ...
                {i===1&&<div style={{ marginTop:'14px', display:'inline-flex', alignItems:'center', gap:'6px', background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.14)', borderRadius:'8px', padding:'5px 10px', fontSize:'11px', color:'#444', fontWeight:600 }}>Tu número · Sin SIM nueva</div>}
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ position:'relative', zIndex:1, maxWidth:'1100px', margin:'0 auto', padding:'0 24px 80px' }}>
          <div style={{ background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'20px', padding:'48px', display:'flex', alignItems:'center', gap:'48px', flexWrap:'wrap' }}>
            <div style={{ flex:1, minWidth:'260px' }}>
              <div style={{ fontSize:'11px', fontWeight:700, letterSpacing:'2px', color:'#888', textTransform:'uppercase', marginBottom:'10px' }}>El diferencial clave</div>
              <h2 style={{ fontSize:'clamp(22px,3vw,34px)', fontWeight:700, letterSpacing:'-.4px', fontFamily:FONT, marginBottom:'14px', lineHeight:'1.2' }}>Tu WhatsApp de siempre.<br/><span style={gradText}>Sin número nuevo.</span></h2>
              <p style={{ fontSize:'15px', color:'#475569', lineHeight:'1.65', marginBottom:'20px' }}>Otros te piden una SIM nueva o un número de empresa. Nosotros no. Escaneas un QR desde tu celular y en 30 segundos tu agente ya está respondiendo desde tu número personal.</p>
            ...
            <div style={{ flexShrink:0, textAlign:'center' }}>
              <div style={{ width:'140px', height:'140px', background:'#fff', borderRadius:'14px', padding:'10px', margin:'0 auto 10px', display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px' }}>
              ...
              <p style={{ fontSize:'13px', color:'#444', fontWeight:600 }}>Escanea · Conecta · Listo</p>
              <p style={{ fontSize:'12px', color:'#94a3b8', marginTop:'3px' }}>30 segundos</p>
            </div>
          </div>
        </section>
        ...
        <section style={{ position:'relative', zIndex:1, maxWidth:'1100px', margin:'0 auto', padding:'0 24px 80px' }}>
          <div style={{ textAlign:'center', marginBottom:'44px' }}>
            <p style={{ fontSize:'11px', fontWeight:700, letterSpacing:'2px', color:'#888', textTransform:'uppercase', marginBottom:'10px' }}>Resultados reales</p>
          ...
          <div className="grid3" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'18px' }}>
            {TESTIMONIOS.map(t => (
              <div key={t.nombre} className="card" style={{ padding:'22px' }}>
                <div style={{ fontSize:'22px', color:'#ddd', marginBottom:'10px', fontWeight:900, lineHeight:1 }}>"</div>
                <p style={{ fontSize:'14px', color:'#334155', lineHeight:'1.65', margin:'0 0 18px', fontStyle:'italic' }}>{t.texto}</p>
                <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                  <div style={{ width:'34px', height:'34px', borderRadius:'50%', background:'rgba(0,0,0,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'13px', fontWeight:700, flexShrink:0 }}>{t.nombre[0]}</div>
                  <div><div style={{ fontSize:'13px', fontWeight:700 }}>{t.nombre}</div><div style={{ fontSize:'12px', color:'#94a3b8' }}>{t.cargo}</div></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <section id="precios" style={{ position:'relative', zIndex:1, maxWidth:'1100px', margin:'0 auto', padding:'0 24px 100px', scrollMarginTop:'80px' }}>
          ...
          <div style={{ textAlign:'center', marginBottom:'52px' }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'transparent', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'999px', padding:'5px 16px', fontSize:'12px', fontWeight:500, color:'#888', marginBottom:'16px' }}>
              Precio
            </div>
          ...
          <div className="pricing-grid">
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              <div style={{ background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'20px', padding:'28px' }}>
                <p style={{ fontSize:'11px', fontWeight:700, color:'#888', letterSpacing:'2px', textTransform:'uppercase', marginBottom:'20px' }}>El math es simple</p>
                <div style={{ display:'flex', flexDirection:'column', gap:'2px' }}>
                  <div className="roi-line" style={{ background:'rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'12px 12px 0 0', borderBottom:'none' }}>
                    <div>
                      <div style={{ fontSize:'13px', fontWeight:600, color:'#1e293b' }}>Comisión promedio por venta</div>
                      <div style={{ fontSize:'11px', color:'#94a3b8', marginTop:'2px' }}>Un Picanto o Sportage vendido</div>
                    </div>
                    <div style={{ fontFamily:FONT_BODY, fontSize:'18px', fontWeight:800, color:'#000', flexShrink:0, marginLeft:'16px' }}>+$500.000</div>
                  </div>
                  <div className="roi-line" style={{ background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:'0', marginBottom:'2px' }}>
                    <div>
                      <div style={{ fontSize:'13px', fontWeight:600, color:'#1e293b' }}>Costo Pulse Motor / mes</div>
                      <div style={{ fontSize:'11px', color:'#94a3b8', marginTop:'2px' }}>Todo incluido, sin sorpresas</div>
                    </div>
                    <div style={{ fontFamily:FONT_BODY, fontSize:'18px', fontWeight:800, color:'#999', flexShrink:0, marginLeft:'16px' }}>−$99.000</div>
                  </div>
                  <div className="roi-line" style={{ background:'rgba(0,0,0,0.06)', border:'1px solid rgba(0,0,0,0.18)', borderRadius:'0 0 12px 12px' }}>
                    <div>
                      <div style={{ fontSize:'14px', fontWeight:700, color:'#0a0a0a' }}>Tu ganancia neta ese mes</div>
                      <div style={{ fontSize:'11px', color:'#444', marginTop:'2px' }}>Con solo 1 lead recuperado</div>
                    </div>
                    <div style={{ fontFamily:FONT_BODY, fontSize:'24px', fontWeight:800, color:'#000', flexShrink:0, marginLeft:'16px' }}>+$401.000</div>
                  </div>
                </div>
                <p style={{ fontSize:'12px', color:'#cbd5e1', marginTop:'16px', lineHeight:1.6 }}>
                  ¿Cuántos leads perdés por semana por no responder a tiempo? Con Pulse Motor, ninguno.
                </p>
              </div>

              {/* Testimonial de refuerzo */}
              <div style={{ background:'rgba(0,0,0,0.025)', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'16px', padding:'20px' }}>
                <div style={{ fontSize:'20px', color:'#ddd', marginBottom:'8px', fontWeight:900, lineHeight:1 }}>"</div>
                <p style={{ fontSize:'14px', color:'#334155', lineHeight:1.65, margin:'0 0 14px', fontStyle:'italic' }}>
                  En el primer mes recuperé 3 ventas que se me hubieran ido. Pulse Motor se pagó solo 15 veces.
                </p>
                <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                  <div style={{ width:'32px', height:'32px', borderRadius:'50%', background:'rgba(0,0,0,0.15)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'13px', fontWeight:700, color:'#000' }}>J</div>
                  <div>
                    <div style={{ fontSize:'13px', fontWeight:700, color:'#1e293b' }}>Jorge P.</div>
                    <div style={{ fontSize:'12px', color:'#94a3b8' }}>Asesor Renault · Medellín</div>
                  </div>
                </div>
              </div>

              {/* Qué cuesta NO tenerlo */}
              <div style={{ padding:'24px', background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.05)', borderRadius:'16px' }}>
                <p style={{ fontSize:'11px', fontWeight:700, color:'#cbd5e1', textTransform:'uppercase', letterSpacing:'1.5px', marginBottom:'16px' }}>¿Qué cuesta NO tener Pulse Motor?</p>
                  ...
                  {[
                    ...
                    <div key={item.label} style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontFamily:FONT, fontSize:'13px', fontWeight:700, color:'#444' }}>{item.valor}</div>
                      </div>
                      <div style={{ fontSize:'11px', color:'#cbd5e1', maxWidth:'100px', textAlign:'right', lineHeight:1.4 }}>{item.desc}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ position:'sticky', top:'90px' }}>
              <div className="pricing-card" style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'24px', padding:'32px', position:'relative', overflow:'hidden' }}>
                <div style={{ display:'inline-flex', alignItems:'center', gap:'6px', background:'rgba(0,0,0,0.06)', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'999px', padding:'4px 12px', fontSize:'11px', fontWeight:700, color:'#444', marginBottom:'20px', letterSpacing:'.5px', textTransform:'uppercase' }}>
                  Plan único · Todo incluido
                </div>

                <div style={{ display:'flex', alignItems:'baseline', gap:'6px', marginBottom:'4px' }}>
                  <span style={{ fontFamily:FONT_BODY, fontSize:'52px', fontWeight:800, color:'#0a0a0a', lineHeight:1 }}>$99k</span>
                  <div>
                    <div style={{ fontSize:'14px', color:'#94a3b8', fontWeight:500 }}>/mes</div>
                    <div style={{ fontSize:'11px', color:'#cbd5e1' }}>COP</div>
                  </div>
                </div>
                <p style={{ fontSize:'13px', color:'#94a3b8', marginBottom:'24px' }}>Cancelás cuando querás · Sin permanencia</p>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginBottom:'28px' }}>
                  {[
                    ...
                    <div key={f} className="check-feat">
                      <div style={{ width:'18px', height:'18px', borderRadius:'50%', background:'rgba(0,0,0,0.08)', border:'1px solid rgba(0,0,0,0.2)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'10px', color:'#000', fontWeight:700 }}>✓</div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                  <button
                    ...
                    style={{ width:'100%', padding:'15px', borderRadius:'8px', border:'1px solid #000', background:'#000', color:'#fff', fontSize:'15px', fontWeight:600, cursor:'pointer', fontFamily:FONT, letterSpacing:'-.2px', transition:'opacity .2s' }}
                    onMouseEnter={e => { e.currentTarget.style.opacity='0.85' }}
                    onMouseLeave={e => { e.currentTarget.style.opacity='1' }}
                  >
                    Lo hacemos por vos — gratis primero
                  </button>
                  <a href="/pulse/signup" style={{ display:'block', width:'100%', padding:'13px', borderRadius:'8px', background:'transparent', border:'1px solid rgba(0,0,0,0.2)', color:'#000', fontSize:'14px', fontWeight:600, textDecoration:'none', textAlign:'center', fontFamily:FONT, transition:'border-color .2s' }}>
                    Empezar solo — 14 días gratis →
                  </a>
                </div>
                <p style={{ fontSize:'11px', color:'#cbd5e1', textAlign:'center', marginTop:'12px' }}>Sin tarjeta para la prueba · Cancelás en 1 clic</p>
              </div>

              <div style={{ marginTop:'14px', display:'flex', alignItems:'center', gap:'12px', padding:'14px 18px', background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.05)', borderRadius:'12px' }}>
                <span style={{ fontSize:'20px', flexShrink:0 }}>🔒</span>
                <div>
                  <div style={{ fontSize:'13px', fontWeight:600, color:'#1e293b' }}>Garantía 7 días</div>
                  <div style={{ fontSize:'12px', color:'#94a3b8', lineHeight:1.5 }}>Si no ves resultados en la primera semana, te devolvemos el dinero. Sin preguntas.</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section style={{ position:'relative', zIndex:1, maxWidth:'680px', margin:'0 auto', padding:'0 24px 100px', textAlign:'center' }}>
          <div style={{ background:'rgba(0,0,0,0.02)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:'20px', padding:'48px 32px' }}>
            ...
            <p style={{ fontSize:'15px', color:'#475569', marginBottom:'28px', lineHeight:'1.65' }}>Con Pulse Motor nunca más. Tu agente responde en 30 segundos, hace seguimiento y agenda citas — mientras vos estás con otro cliente.</p>
            <a href="/pulse/signup" style={{ display:'inline-block', padding:'15px 32px', borderRadius:'8px', background:'#000', color:'#fff', fontSize:'16px', fontWeight:600, textDecoration:'none', fontFamily:FONT, letterSpacing:'-.2px' }}>Crear mi agente gratis →</a>
            <p style={{ fontSize:'13px', color:'#cbd5e1', marginTop:'14px' }}>14 días gratis · Sin tarjeta · Tu WhatsApp actual</p>
          </div>
        </section>

        <footer style={{ position:'relative', zIndex:1, padding:'24px', textAlign:'center', fontSize:'13px', color:'#cbd5e1', borderTop:'1px solid rgba(0,0,0,0.05)' }}>
