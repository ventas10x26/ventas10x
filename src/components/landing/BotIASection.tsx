...
export default function BotIASection() {
  ...
  return (
    <div id="bot-ia" style={{
      ...
      <div className="bot-grid" style={{ maxWidth: '1100px', margin: '0 auto', padding: '6rem 2rem' }}>

        {/* TEXTO */}
        <div>
          <div style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '.12em', color: '#FF6B2B', marginBottom: '1rem', textTransform: 'uppercase' }}>Bot IA</div>
          <h2 style={{ fontSize: 'clamp(28px,4vw,44px)', fontWeight: 800, letterSpacing: '-.03em', marginBottom: '1.5rem', lineHeight: 1.1, color: '#fff' }}>
            Tu vendedor más productivo trabaja 24/7
          </h2>
          <p style={{ fontSize: '16px', color: 'rgba(255,255,255,.55)', lineHeight: 1.8, marginBottom: '2rem', fontWeight: 400 }}>
          ...
          <div style={{ marginBottom: '2rem', background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: '16px', padding: '1.25rem' }}>
            <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '.1em', color: 'rgba(255,255,255,.35)', marginBottom: '1rem', textTransform: 'uppercase' }}>Variables que captura el bot</div>
            <div className="variables-grid">
              {[
                ...
                <div key={v.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '13px', color: 'rgba(255,255,255,.6)', fontWeight: 500 }}>{v.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            <div style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(255,255,255,.3)', marginBottom: '.75rem', letterSpacing: '.08em', textTransform: 'uppercase' }}>Usado por asesores como</div>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '24px', padding: '0', boxShadow: '0 20px 60px rgba(0,0,0,.35)', minHeight: '420px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', background: '#f9fafb', borderBottom: '1px solid #f0f0f0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: 700, color: esTuNombre ? '#FF6B2B' : '#111', transition: 'color 0.4s ease', letterSpacing: '-.01em' }}>
                  Bot IA · {asesor.nombre}
                </div>
                <div style={{ fontSize: '11px', color: '#16a34a', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 500 }}>
                  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#16a34a', display: 'inline-block' }} />
                  Perfilando lead...
                </div>
              </div>
            </div>
            <div style={{ fontSize: '11px', color: '#9ca3af', background: '#f3f4f6', padding: '4px 10px', borderRadius: '10px', fontWeight: 600 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '1.25rem', flex: 1, overflowY: 'hidden' }}>
              ...
              return (
                <div key={`${loopCount}-${origIdx}`} style={{ display: 'flex', justifyContent: isBot ? 'flex-start' : 'flex-end', animation: 'slideIn 0.5s ease' }}>
                  <div style={{ maxWidth: '84%', background: isBot ? '#FF6B2B' : '#f3f4f6', borderRadius: isBot ? '4px 16px 16px 16px' : '16px 4px 16px 16px', padding: '10px 14px', boxShadow: isBot ? '0 2px 8px rgba(255,107,43,.25)' : '0 1px 3px rgba(0,0,0,.06)' }}>
                    {isBot && (
                      <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '.07em', marginBottom: '4px', color: 'rgba(255,255,255,.7)', textTransform: 'uppercase' }}>
                        {asesor.nombre}
                      </div>
                    )}
                    <div style={{ fontSize: '13px', color: isBot ? '#fff' : '#1f2937', lineHeight: 1.6, fontWeight: isBot ? 500 : 400 }}>
                      {m.msg}
                    </div>
                  </div>
                </div>
            ...
            {visible.length > 0 && visible.length < conversation.length && (
              <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                <div style={{ background: '#f3f4f6', borderRadius: '4px 16px 16px 16px', padding: '12px 16px', display: 'flex', gap: '5px', alignItems: 'center' }}>
                  {[0, 1, 2].map(d => (
