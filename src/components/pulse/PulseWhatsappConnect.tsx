...
// Componente de conexión WhatsApp con QR scanner para el dashboard
...
export function PulseWhatsappConnect({ email, onConnected }: Props) {
  ...
  // Polling mientras está en qr_ready
  useEffect(() => {
  ...
  return (
    <div style={{ background: 'rgba(15,23,42,0.5)', border: '1px solid #1e293b', borderRadius: 16, padding: 24, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 9, fontWeight: 800, color: estado === 'connected' ? '#10b981' : '#f97316', background: estado === 'connected' ? 'rgba(16,185,129,0.12)' : 'rgba(249,115,22,0.12)', border: `1px solid ${estado === 'connected' ? 'rgba(16,185,129,0.3)' : 'rgba(249,115,22,0.3)'}`, padding: '4px 10px', borderRadius: 999 }}>
      ...
      {estado === 'no_instance' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <p style={{ fontSize: 14, color: '#94a3b8', margin: 0, lineHeight: 1.6 }}>
            Conecta tu WhatsApp actual para que el agente IA responda automáticamente a tus leads. <strong style={{ color: '#e2e8f0' }}>Sin número nuevo, sin SIM card</strong> — solo escaneas un QR desde tu teléfono.
          </p>
      ...
      {estado === 'qr_ready' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              {[
                ...
                'Apunta la cámara al QR de esta pantalla',
              ].map((paso, i) => (
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={verificarEstado} style={{ padding: '8px 16px', borderRadius: 8, border: `1px solid ${C.blue}`, background: 'rgba(14,165,233,0.1)', color: C.blue, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              ↻ Verificar conexión
            </button>
            <button onClick={conectar} disabled={creando} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #334155', background: 'transparent', color: '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              Regenerar QR
            </button>
          </div>
      ...
      {/* LOADING */}
      {estado === 'loading' && (
        <p style={{ fontSize: 13, color: '#64748b', margin: 0 }}>Verificando estado de conexión…</p>
      )}
