...
export async function manejarMensajeEntranteFenix(
  ...
  // Se guarda el mensaje del usuario de una vez -- eso sí ya pasó,
  // independientemente de si logramos responder.
  let historialFinal: MensajeHistorial[] = [...historial, { role: 'user', content: texto }]
  ...
  if (respuesta) {
    ...
    try {
      ...
      // Solo se guarda como "enviado" en el historial (burbuja verde en el
      // admin) si Graph API de verdad lo aceptó -- antes se guardaba ANTES
      // de intentar el envío, así que un fallo de Graph API (número fuera
      // de ventana de 24h, token vencido, etc.) quedaba invisible: el panel
      // mostraba el mensaje como enviado aunque nunca le llegara a nadie.
      historialFinal = [...historialFinal, { role: 'assistant', content: respuesta }]
    } catch (e) {
      console.error('[fenix cloud handler] enviarTexto error (respuesta no entregada):', e)
    }
  }

  await guardarConversacion(from, historialFinal, cuenta.phone_number_id)
}
